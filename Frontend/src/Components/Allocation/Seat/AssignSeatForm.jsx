...
export default function AssignSeatForm({ isOpen, onClose , seatNumber }) {
    ...
    function initialUsersData() {
        const users = allUsers.filter((user) => user?.user?.seatId == null && user?.user?.seatType == "Fixed")
    ...
    const handleSubmit = async (e) => {
        ...
        if (!seatNumber) {
            return setError('Please select a Seat Number.');
        }

        console.log(formData)
        setSpinnerOpen(true)
        try {
            const res = await assignSeat(formData)
            if(res?.data?.status){
                            ...
                            <div className="p-5 flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    <div className="p-2 bg-blue-500/10 rounded-lg">
                                        ...
                                        <div className="text-blue-500  px-2">{seatNumber}</div>
                                    </div>
                                    <div>
                                        <h2 className="text-slate-100 font-bold text-lg leading-tight">Seat Number</h2>
                                        <p className="text-slate-500 text-xs mt-0.5">#Assign Seat</p>
                                    </div>
                                </div>
