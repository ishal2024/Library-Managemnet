...
export default function AssignedLockerOverviewModal({ 
  isOpen, 
  onClose,
  lockerData 
}) {
              ...
              <div className="bg-[var(--bg-surface)]/40 border border-[var(--border-muted)] rounded-xl p-1 flex flex-col">
                ...
                <div className="p-4 flex items-center gap-4 border-b border-[var(--border-muted)]/60 bg-[var(--bg-card)]/50 rounded-t-lg">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[var(--accent-primary)]/20 to-purple-500/20 border border-[var(--accent-primary)]/30 flex items-center justify-center text-[var(--accent-primary)] font-bold text-lg shrink-0 shadow-inner">
                    {getInitials(lockerData?.userDetail?.name)}
                  </div>
                  <div className="flex flex-col">
                    <h3 className="text-base font-bold text-[var(--text-primary)] leading-tight">{lockerData?.userDetail?.name}</h3>
                    <p className="text-sm text-[var(--text-secondary)] mt-0.5">{lockerData?.userDetail?.contact}</p>
                  </div>
                </div>

                {/* Compact Details Rows */}
                <div className="p-4 flex flex-col gap-3.5">
                  <DetailRow icon={<MapPin />} label="Address" value={lockerData?.userDetail?.address} />
                  <DetailRow icon={<Calendar />} label="Assigned Date" value={new Date(lockerData?.locker?.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })} />
                </div>
